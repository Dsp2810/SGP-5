...
const { generateTemplateHTML } = require('../templates/portfolioTemplates');
...
exports.deployPortfolio = async (req, res) => {
  try {
    ...
    // Generate portfolio URL (username-based for clean URLs)
...
/**
 * Get portfolio by username (for public viewing via /username URL)
 */
exports.getPortfolioByUsername = async (req, res) => {
  try {
    const { username } = req.params;

    // Find the latest published portfolio for this username
 ...
 */
exports.deployToVercel = async (req, res) => {
  try {
    const { portfolioId } = req.params;
    const userId = req.user.id;

    console.log(`[Vercel Deploy] Starting for portfolioId=${portfolioId}, userId=${userId}`);

    // Find portfolio
    const portfolio = await Portfolio.findOne({ portfolioId, userId });
    if (!portfolio) {
      console.log('[Vercel Deploy] Portfolio not found');
      return res.status(404).json({ success: false, message: 'Portfolio not found' });
    }

    const vercelToken = process.env.VERCEL_TOKEN;
    if (!vercelToken) {
      console.error('[Vercel Deploy] VERCEL_TOKEN missing from env! Restart the server after adding it to .env');
      return res.status(500).json({
        success: false,
        message: 'Vercel token not configured. Please restart the backend server after adding VERCEL_TOKEN to .env.'
      });
    }

    // Flatten portfolio data for the template
    const data = {
    ...
    let htmlContent;
    try {
      htmlContent = generateTemplateHTML(data, portfolio.template || 'template1');
      console.log(`[Vercel Deploy] HTML generated successfully (${htmlContent.length} chars)`);
    } catch (htmlError) {
      console.error('[Vercel Deploy] HTML generation failed:', htmlError);
      return res.status(500).json({
        success: false,
        message: 'Failed to generate portfolio HTML: ' + htmlError.message
      });
    }

    // Create a clean project name using username for a clean Vercel URL
    // Vercel project names: lowercase, alphanumeric, hyphens only, max 100 chars
    ...
    console.log(`[Vercel Deploy] Sending to Vercel API... project=${projectName}`);
    ...
    if (!vercelResponse.ok) {
      console.error('[Vercel Deploy] Vercel API error:', JSON.stringify(vercelResult));
      return res.status(500).json({
        success: false,
    ...
    console.log(`[Vercel Deploy] Success! URL: https://${vercelResult.url}`);

    // Update portfolio with Vercel deployment details
    ...
    console.error('[Vercel Deploy] Unexpected error:', error.message, error.stack);
    res.status(500).json({
      success: false,
...
/**
 * Generate portfolio HTML (legacy)
 */
