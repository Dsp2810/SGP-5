    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-cyan-50">
      ...
      <header className="bg-gradient-to-r from-blue-600 to-cyan-600 text-white py-16">
        <div className="container mx-auto px-6 text-center">
          <div className="w-32 h-32 bg-white rounded-full mx-auto mb-6 flex items-center justify-center text-blue-600 text-5xl font-bold shadow-xl">
            {data.name?.charAt(0) || 'P'}
          </div>
          <h1 className="text-5xl font-bold mb-3">{data.name || 'Your Name'}</h1>
          <p className="text-2xl text-blue-100 mb-4">{data.title || 'Your Title'}</p>
          ...
          <div className="container mx-auto px-6 max-w-4xl">
            ...
            <div className="flex flex-wrap gap-3">
              {data.skills.map((skill, index) => (
                <span key={index} className="px-6 py-3 bg-gradient-to-r from-blue-500 to-cyan-500 text-white rounded-full font-semibold shadow-md hover:shadow-lg transition-shadow">
                  {skill}
                </span>
              ))}
            </div>
          ...
          <div className="container mx-auto px-6 max-w-4xl">
            ...
            <div className="space-y-8">
              {data.experience.map((exp, index) => (
                <div key={index} className="border-l-4 border-blue-500 pl-6 hover:border-cyan-500 transition-colors">
      ...
      <footer className="bg-gradient-to-r from-blue-600 to-cyan-600 text-white py-8">
        <div className="container mx-auto px-6 text-center">
          <div className="flex justify-center gap-6 mb-4">
            {data.github && <a href={data.github} className="hover:text-blue-200">GitHub</a>}
            {data.linkedin && <a href={data.linkedin} className="hover:text-blue-200">LinkedIn</a>}
          </div>
          <p className="text-blue-100">© 2025 {data.name}. All rights reserved.</p>
        </div>
      </footer>
    </div>
  );
